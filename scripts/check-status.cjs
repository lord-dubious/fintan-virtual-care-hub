...
const ACTUAL_FRONTEND_PORT =
  envVars.VITE_PORT || process.env.VITE_PORT || "10000";
const ACTUAL_BACKEND_PORT = envVars.PORT || process.env.PORT || "3000";

// Dynamic host detection for status checks
const getHost = () => {
  return envVars.BACKEND_HOST || process.env.BACKEND_HOST || "localhost";
};
...
const SERVICES = {
  frontend: {
    ...
    url: `http://${host}:${ACTUAL_FRONTEND_PORT}`,
    healthCheck: `http://${host}:${ACTUAL_FRONTEND_PORT}`,
  },
  backend: {
    name: "Backend (Express)",
    port: ACTUAL_BACKEND_PORT,
    url: `http://${host}:${ACTUAL_BACKEND_PORT}`,
    healthCheck: `http://${host}:${ACTUAL_BACKEND_PORT}/health`,
  },
};
...
async function checkService(service) {
  try {
    const { stdout, stderr } = await execAsync(
      `curl -s -o /dev/null -w "%{http_code}" ${service.healthCheck}`,
      {
        timeout: 3000,
      }
    );
